...
public partial class RecipePage
{
    ...
    private readonly List<Label> ingredentPortionLabels = new();
    ...
    public int Poritons
    {
        ...
        set
        {
            if (value <= 10 && value >= 1) portions = value;
        }
    }
    ...
    private async void GET_recipeDto(string id)
    {
        try
        {
            var response = await httpClient.GetAsync(RamseyApi.V2.Recipe.Retreive + "?id=" + id);
            ...
            {
                await DisplayAlert("Fel",
                    "Kunnde inte ansluta till servern\n\nstatus code: " + (int)response.StatusCode, "ok");
            }
        }
        catch (Exception)
        {
    ...
    {
        //Ingredients
        for (var i = 0; i < recipe.RecipeParts.Count() + 1; i++)
        ...
        for (var i = 0; i < recipe.RecipeParts.Count(); i++)
        {
            // quantity and unit
        ...
        if (recipe.Owner == RecipeProvider.Hemmets) startsWithNum = true;
        var showNumbers = true;
        if (recipe.Directions.Count() <= 1) showNumbers = false;


        var n = 1;
        for (var i = 0; i < recipe.Directions.Count(); i++)
        {
            var text = recipe.Directions.ToList()[i].TrimStart();
            ...
            if (!startsWithNum || char.IsDigit(text[0]))
            {
                ...
                while (char.IsDigit(text[0]) || text[0] == '.') text = text.Substring(1);
                text = text.TrimStart();
            ...
            else
            {
                ...
                StackInstructions.Children.Add(new Label
                {
                    ...
                    FontSize = Constants.fontSize2,
                    Margin = new Thickness(Constants.TextListMargin, Constants.Padding3, Constants.TextListMargin,
                        Constants.TextListMargin),
    ...
    {
        var portionMuliplier = 0.5;
        LabelPortions.Text = portions == 1 ? portions + " portion" : portions + " portioner";
        for (var i = 0; i < ingredentPortionLabels.Count; i++)
            ingredentPortionLabels[i].Text =
                recipe.RecipeParts.ToList()[i].Quantity != 0 && recipe.RecipeParts.ToList()[i].Unit != "odefinierad"
                    ? (recipe.RecipeParts.ToList()[i].Quantity * portions * portionMuliplier).ToString().Trim() + " " +
                      recipe.RecipeParts.ToList()[i].Unit.Trim()
                    : "";
    }
